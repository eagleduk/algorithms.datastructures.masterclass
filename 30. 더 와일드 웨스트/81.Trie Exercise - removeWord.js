class Trie {
  ...
  removeWord(word) {
    ...
    var obj;
    while (wIndex > 0) {
      obj = this.findWord(word.slice(0, wIndex), 0);
      if (Object.keys(obj.characters).length > 1) break;

      wIndex--;
    }

    if (wIndex === word.length) obj.isWord = false;
    else delete obj.characters[word[wIndex]];
  }
}
